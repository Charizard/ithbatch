...
<?php
  ...
  $password = getenv('MYSQL_PASSWORD'));*/
	...
	if(isset($_POST['submit'])){
		...
		mysql_query("INSERT INTO comments (articleid,name,mail,comment) VALUES('".$_GET['id']."','".$_SESSION['fb_name']."','".$_SESSION['fb_mail']."','".$_POST['comment']."')");
	
	}
	echo "<title>";
	$result=mysql_query("SELECT * FROM artiles WHERE id='".$_GET['id']."'");
...
<script type="text/javascript" src="script/post-jquery.js" ></script>
<link rel="stylesheet" href="themes/owntheme/poststyle.css"  />
</head>
<body id="body">
<center>
<div id="menus" class="menubar">
	<ul class="menugroup">
            	<li class="menulink">
                	<a id="home" href="#" onClick="homelink()">home</a>
                </li>
                <li class="menulink" >
                	<a id="members" href="#" onClick="memberlink()">members</a>
                </li>
                <li class="menulink" >
                	<a id="about" href="#" onClick="aboutlink()">about</a>
                </li>
            	<li class="menulink" >
                	<a id="contact" href="#" onClick="contactlink()">contact</a>
                </li>
    </ul>
</div>
...
<div id="base">
	<div id="commentwrapper">
    	<div id="usercomment">
        	...
        	<div id="commentform">
            	<form action="" method="post" onsubmit="return checkfields()">
                	<br/><div id="name"><?php echo "Logged in as <a href='http://www.facebook.com/".$user_profile['id']."' target='_blank'>".$user_profile['name']."</a>"; ?></div>
                    <textarea class="textarea" id="comment" name="comment" cols="40" rows="15" placeholder="Your Comment" ></textarea><br/>
                    <input class="submitbutton" type="submit" name="submit" value="Comment" />
                </form>
            </div>
        </div>
        <div id="displaycomments">
        	Comments
            <?php
				$result=mysql_query("SELECT * FROM comments WHERE articleid='".$_GET['id']."' ORDER BY timestamp");
				if(!$result)
					echo "<b>No Comments</b>";
				while($row=mysql_fetch_array($result))
				{
					echo "<div class='comments'>\n<div id='usernamewrapper'><div id='username'>".$row['name']."<br/>(".$row['mail'].")</div></div>\n<div id='timestamp'>at ".$row['timestamp']."</div>\n<div id='commentvalue'>".$row['comment']."</div>\n</div>\n";
				}
            ?>
        </div>
    </div>
